...
public class EmployeService  implements IDao<Employe> {
    ...
    public Employe findById(int id) {
      ...
      /*  public  List<Projet> getProjetList(int id) {
        List<Projet> pListe=null;
        Session session = null;
        Transaction tx = null;
        try {
            session = HibernateUtil.getSessionFactory().openSession();
            tx = session.beginTransaction();
            String hql = "SELECT et FROM Projet WHERE emplye_id = :id";
            pListe = session.createQuery(hql).setParameter("id",id).list();
            tx.commit();
        } catch (HibernateException ex) {
            if (tx != null) {
                tx.rollback();
     ...
     public void afficherTachesEmploye(int employeId) {
        ...
        if (tacheList.isEmpty()) {
            System.out.println("Aucune tache realisee");
        } else {
        ...
        for (Projet p : ps.findAll() ){
            if (p.getEmploye().getId()== employeId) {
             ...
             System.out.println("Aucune Projets realisee");
            }
        }}}
